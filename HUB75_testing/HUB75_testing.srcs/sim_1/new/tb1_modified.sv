...
// THIS is a modified version of tb1 which implements the memory_intitialization module instead of the simulation only memory write in tb1

module tb1_modified #(BITS_PER_PIXEL=16);
    logic n_reset;
    logic clk;
    logic [1:0] hub75_red;
    ...
    always @ (negedge hub75_addr[4]) begin  //this will trigger immediately by default for some reason so
        if(n_reset && mem_write_done) begin
                for (integer y_count = 0; y_count < 64; y_count++) begin
                    for (integer x_count = 0; x_count < 64; x_count++) begin
