...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	if(XSpi_Transfer(&SpiInstance, writeBuffer, readBuffer, 2)!=0) {
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	BYTE writeBuffer[nbytes + 1 ]; //+1 is for the reg
	BYTE readBuffer[nbytes + 1];		//+1 for the reg
	...
	if(XSpi_Transfer(&SpiInstance, writeBuffer, readBuffer, nbytes + 1) != 0) {
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	BYTE writeBuffer[nbytes + 1]; //+1 for reg
	BYTE readBuffer[nbytes + 1]; //+1 for reg
